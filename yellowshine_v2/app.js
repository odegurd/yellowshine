...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildHeroSlider(bestSellers) {
    ...
    function goSlide(n) {
      sliderWrap.querySelectorAll('.slide')[idx]?.classList.remove('active');
      dotsWrap.querySelectorAll('.dot')[idx]?.classList.remove('active');
      idx = n;
      sliderWrap.querySelectorAll('.slide')[idx]?.classList.add('active');
      dotsWrap.querySelectorAll('.dot')[idx]?.classList.add('active');
    }

    if (items.length > 1) setInterval(() => goSlide((idx + 1) % items.length), 2800);
  }
  ...
  function attachOrderBtns() {
    document.querySelectorAll('.order-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        window.open(`https://wa.me/62881026842357?text=${msg}`, '_blank');
      });
    });
  }
