...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.confirmDelete = function(id) {
    ...
    showConfirm(`Hapus "${p?.name}"?`, 'Produk akan dihapus permanen.', () => {
      const updated = YS.getProducts().filter(x => x.id !== id);
      YS.saveProducts(updated);
      dispatchProductsChange();
  ...
  // ===== DISPATCH STORAGE EVENT (syncs open index.html tabs) =====
  function dispatchProductsChange() {
    // Manually fire a storage event for same-tab listeners on index.html
    // (localStorage events only fire in OTHER tabs natively)
    // We store a timestamp so the event key changes each time
    localStorage.setItem('ys_products_updated', Date.now().toString());
  }
